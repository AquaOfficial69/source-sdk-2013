...
public:
		...
		if (bBase && m_ResponseStrings.Count() <= 0)
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				for (int i2 = 0; i2 < count2; i2++)
				{
					...
					m_Responses[i].group[i2].value = ResponseCopyString((CNumStr( index ).String()));
				}
			}
		}
	...
	CUtlVector<const char*> m_ResponseStrings;
};
...
bool LoadCustomXenRecipeFile(const char *scriptfile)
{
	...
	xsys->LoadFromBuffer(scriptfile, (const char *)buf.PeekGet());
